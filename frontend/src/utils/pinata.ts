...
const PINATA_JTW = import.meta.env.VITE_PINATA_JWT || "";
...
export async function uploadFile(image_file_obj: File) {
  try {
    const upload = await pinata.upload.file(image_file_obj);
    console.log(upload);
...
export async function getMetadata(hash: string) {
