...
import { NFTLottery, Lazy1155 } from "typechain-types"
...
describe("Lottery", async function () {
	...
	beforeEach(async function () {
		...
		for (let tokenId = 0, quantity = 1 * factor; tokenId < nftTypes; tokenId++) {
			...
			await lazy1155.connect(owner).mint(owner, tokenId, quantity, metadata)
			//const total_supply = await lazy1155["totalSupply(uint256)"](tokenId)
			expect(await lazy1155.balanceOf(owner, tokenId)).to.be.equal(quantity)
			quantity *= 2
		}
		...
		const result: ContractTransactionResponse = await lazy1155
			.connect(owner)
			.setApprovalForAll(lotteryContract, true)
		await result.wait()
		const check = await lazy1155.connect(owner).isApprovedForAll(owner, lotteryContract)
		expect(check).to.be.equal(true)

		for (let tokenId = 0, quantity = 1 * factor; tokenId < nftTypes; tokenId++) {
			// const total_supply = await nftContract["totalSupply(uint256)"](tokenId)
			await lazy1155.safeTransferFrom(owner, lotteryContract, tokenId, quantity, metadata)
			quantity *= 2
		}
	})
	...
	describe("claimNFT", () => {
		...
		// 	expect(resultEvents[1]).to.be.equal(3) // consolidation Prize
	...
	describe("Draw", function () {
		...
		it("Player draw successful Third Prize", async function () {
			...
			expect(resultEvents[1]).to.be.equal(true)
			// and nftId should be Case 3
			expect(resultEvents[2]).to.be.equal(2)
			// and total draws equals 1
			expect(resultEvents[3]).to.be.equal(1)
		})
		...
		it("Player draw fail", async function () {
			...
			// and nftId should be be 1 Second Prize since distance = 1
			expect(resultEvents[2]).to.be.equal(1)
