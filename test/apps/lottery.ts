...
import Lottery from "../build/NFTLottery/NFTLottery.abi.json";
import { Contract } from "ethers";
import { expect } from "chai";
...
describe("Lottery", async function () {
  ...
  beforeEach(async function () {
    ...
    // Setting probability
    await fairyRingContract.mock.latestRandomness.returns(
      getRandomNumber(factor),
    );

    lotteryContract = await deployContract(owner, Lottery, [
      fairyRingContract.address,
    ]);
  });
  ...
  describe("Draw", function () {
    ...
    it("Player draw fail", async function () {
      ...
      const result = await lotteryContract
        .connect(user1)
        .draw(getRandomNumber(19));
  ...
  describe("PlayersName", function () {
    ...
    it("Player Name set should be with maximun 7 chars long utf8 with . allowed", async function () {
      ...
      let result = await lotteryContract
        .connect(user1)
